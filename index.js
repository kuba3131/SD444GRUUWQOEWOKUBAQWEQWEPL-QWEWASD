...
.setTitle('Sorgu Botu!')
  ...
  .setCustomId('adres')
  .setLabel('Adr4s için T-Є GİRİNİZ')
...
client.on('interactionCreate', async (interaction) => {
	...
	if(interaction.customId === "başvuru3"){
...
let rol = db.fetch(`basvururol_${interaction.guild.id}`)
		...
		const adres = interaction.fields.getTextInputValue('adres')
    ...
    await interaction.reply({ content: 'Bot bakıma alındı.', ephemeral: true });
